...
useEffect(()=>{
    ...
    const handleSubmit=(e)=>{
        ...
        // console.log(`Use Registration: {email:${email}} {password:${password}}`);
    ...
    <div className="flex">
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-md  bg-white p-8 rounded-lg border shadow-sm">
            ...
            <p className="mt-6 text-center text-sm">
                ...
                <Link to={`/register?redirect=${encodeURIComponent(redirect)}`} className="text-blue-600 hover:text-blue-800 transition"> Register </Link>
            </p>
        </form>
        </div>
        <div className="hidden md:block w-1/2 bg-gray-800">
        <div className="h-full flex flex-col justify-center items-center">
            <img src={login} alt="Log In To Account" className="h-[750px] w-full object-cover" />
        </div>
        </div>
    </div>
  )
